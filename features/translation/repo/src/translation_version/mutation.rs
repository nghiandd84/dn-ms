...
use features_translation_entities::translation_version::{
    ActiveModel, Column, Entity, Model, ModelOptionDto, TranslationVersionForCreateDto,
...
impl TranslationVersionMutation {
    pub fn create_translation_version<'a>(
        ...
        TranslationVersionMutationManager::create_uuid(data.into())
    }
    ...
    pub fn update_translation_version<'a>(
        ...
        TranslationVersionMutationManager::update_by_id_uuid(version_id, data.into())
    }

    pub fn delete_translation_version<'a>(
        version_id: Uuid,
    ) -> impl std::future::Future<Output = Result<bool, DbErr>> + 'a {
        TranslationVersionMutationManager::delete_by_id_uuid(version_id)
    }
}
