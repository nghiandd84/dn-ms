...
impl KeyTagQueryManager {
    pub async fn get_tags_by_key_id(
        db: &DbConn,
        key_id: Uuid,
    ) -> Result<Vec<KeyTagData>, AppError> {
        ...
        let query_result = Self::filter(db, &paging, &order, &filters).await?;
    ...
    pub async fn key_tag_exists(db: &DbConn, key_id: Uuid, tag_id: Uuid) -> Result<bool, AppError> {
        let paging = Pagination::new(1, 100); // Assuming a reasonable limit for tags
        let order = Order::default();
        let key_id_filter = FilterEnum::Uuid(FilterParam {
            name: Column::KeyId.to_string(),
