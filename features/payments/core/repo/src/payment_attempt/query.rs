...
use features_payments_core_entities::payment_attempt::{ActiveModel, Column, Entity, ModelOptionDto};
...
impl PaymentAttemptQuery {
    pub async fn get_payment_attempt_by_id(
        ...
        let model = PaymentAttemptQueryManager::get_by_id_uuid(db, attempt_id).await?;
        Ok(model.into())
    }
    ...
    pub async fn get_payment_attempts<'a>(
        ...
        let result = PaymentAttemptQueryManager::filter(db, pagination, order, filters).await?;
