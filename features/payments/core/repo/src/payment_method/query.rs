...
use features_payments_core_entities::payment_method::{ActiveModel, Column, Entity, ModelOptionDto};
...
impl PaymentMethodQuery {
    pub async fn get_payment_method_by_id(
        ...
        let model = PaymentMethodQueryManager::get_by_id_uuid(db, method_id).await?;
        Ok(model.into())
    }
    ...
    pub async fn get_payment_methods<'a>(
        ...
        let result = PaymentMethodQueryManager::filter(db, pagination, order, filters).await?;
