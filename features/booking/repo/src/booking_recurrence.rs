...
use features_booking_entities::booking_recurrence::{
    BookingRecurrenceForCreateDto, Entity, Model, ModelOptionDto,
};
use features_booking_model::booking_recurrence::BookingRecurrenceData;

pub struct BookingRecurrenceMutation;

impl BookingRecurrenceMutation {
    /// Insert a recurrence row within a transaction. PK is `booking_id`.
    pub async fn create_with_txn(
        data: BookingRecurrenceForCreateDto,
...
pub struct BookingRecurrenceQuery;

impl BookingRecurrenceQuery {
    pub async fn get_by_booking_id(
        booking_id: Uuid,
    ) -> Result<Option<BookingRecurrenceData>, AppError> {
