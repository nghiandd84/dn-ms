...
use features_auth_entities::scope::Model as ScopeModel;
...
fn sample_model() -> ScopeModel {
    ScopeModel {
        id: Uuid::new_v4(),
        name: "read:users".to_string(),
        description: "Read access to users".to_string(),
...
fn build_app() -> Router {
    ...
    let cache = Cache::<String, AuthCacheState>::new("redis://127.0.0.1/", "test_scope")
        .expect("Failed to create cache");
    let app_state = AppState::new(&db_conn, cache, Some(AuthAppState::default()));

    api_auth::routes::scope::routes(&app_state)
...
#[tokio::test]
async fn test_create_scope_requires_auth() {
    let app = build_app();
...
#[tokio::test]
async fn test_update_scope_requires_auth() {
    let app = build_app();
    let id = Uuid::new_v4();
...
#[tokio::test]
async fn test_delete_scope_requires_auth() {
    let app = build_app();
    let id = Uuid::new_v4();
...
#[tokio::test]
async fn test_get_scope_requires_auth() {
    let app = build_app();
    let id = Uuid::new_v4();
...
#[tokio::test]
async fn test_filter_scopes_requires_auth() {
    let app = build_app();
...
#[tokio::test]
async fn test_create_scope_with_admin() {
    let app = build_app();
...
#[tokio::test]
async fn test_delete_scope_with_admin() {
    let app = build_app();
    let id = Uuid::new_v4();
...
#[tokio::test]
async fn test_create_scope_validation_short_name() {
    let app = build_app();
...
#[tokio::test]
async fn test_create_scope_invalid_json() {
    let app = build_app();
