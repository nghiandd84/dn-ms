...
use features_auth_entities::role::Model as RoleModel;
...
fn build_app() -> Router {
    ...
    let cache = Cache::<String, AuthCacheState>::new("redis://127.0.0.1/", "test_role")
        .expect("Failed to create cache");
    let app_state = AppState::new(&db_conn, cache, Some(AuthAppState::default()));

    api_auth::routes::role::routes(&app_state)
...
#[tokio::test]
async fn test_update_role_requires_auth() {
    let app = build_app();
    let id = Uuid::new_v4();
...
#[tokio::test]
async fn test_filter_roles_requires_auth() {
    let app = build_app();
...
#[tokio::test]
async fn test_assign_permissions_requires_auth() {
    let app = build_app();
    let id = Uuid::new_v4();

    let body = json!({ "permission_ids": [Uuid::new_v4()] });

    let req = Request::builder()
        .method(Method::POST)
        .uri(format!("/roles/{}/assign-permissions", id))
        .header(header::CONTENT_TYPE, "application/json")
        .body(Body::from(serde_json::to_string(&body).unwrap()))
        .unwrap();
...
#[tokio::test]
async fn test_unassign_permissions_requires_auth() {
    let app = build_app();
    let id = Uuid::new_v4();
    ...
    let req = Request::builder()
        .method(Method::POST)
        .uri(format!("/roles/{}/unassign-permissions", id))
        .header(header::CONTENT_TYPE, "application/json")
        .body(Body::from(serde_json::to_string(&body).unwrap()))
        .unwrap();
...
#[tokio::test]
async fn test_create_role_with_admin() {
    let app = build_app();
...
#[tokio::test]
async fn test_delete_role_with_admin() {
    let app = build_app();
    let id = Uuid::new_v4();
...
#[tokio::test]
async fn test_create_role_validation_empty_name() {
    let app = build_app();
...
#[tokio::test]
async fn test_create_role_validation_empty_description() {
    let app = build_app();
...
#[tokio::test]
async fn test_create_role_validation_missing_client_id() {
    let app = build_app();
...
#[tokio::test]
async fn test_create_role_invalid_json() {
    let app = build_app();
